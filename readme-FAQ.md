...
Медицинский персонал должен оказывать услуги в соответствии с прайс-листом.
...
**СТЕК**
SpringBoot, Hibernate, Rest, Spring Data JPA, HQL, SQL, PostgreSQL, FlyWay, Lombok, JUnit, Swagger, Git
...
Если прямо посередине экрана возле надписи Active Profiles вы не видите Environment variables, то
...
Заходим в него и прописываем переменные. Переменные прописываем в строковом поле, а не в редакторе. В редакторе они могут отображаться не все.
Сохраняем.
...
На фронт отправляем Dto. В мануале по стеку есть описание, что такое Dto и как с ним работать.
...
Если ваш код явно вносит изменения в выполнение других тестов и вы !! понимаете !!, 
почему происходит та или иная правка и она не ломает сути теста, правите тесты которые сломались.
...
Если не уверены сможете ли вы разрешить конфликты своей измененной ветки с веткой dev, то выгрузите все свои изменения на полку.
Затем загрузите последние изменения с dev в свою ветку и внедряйте изменения по задаче доставая их с полки
...
Перед пушем своей ветки нужно сделать следующее:
1) Вливаем все актуальные файлы из основной ветки dev в свою
2) Делаем Maven Clean
3) Запускаем проект – должен запуститься
4) Выполняем все тесты - должны завершиться успешно
5) Убираем * из импортов (import all). Нам не нужно загружать классы, с которыми мы не работаем.

Чтобы импорты не сворачивались в звездочку:
1) Заходим в IDEA в File -> Setting -> Editor -> Code Style -> Java -> Import
2) В строке Class count to user import with '*' в квадрате пишем 500
3) В строке Names count to user static import with '*' в квадрате пишем 500
4) Нажимаем Apply и ОК.
5) Дополнительно в настройках коммита (шестеренка возле слова amend) можно включить Optimize import

**ЧАСТЫЕ ОШИБКИ**

- Нейминг.
Если вы создали ссылку, то по ее имени должно быть понятно, на что она указывает. Не создавайте короткие ссылки, понятные только вам.
Если вы создали метод, по его имени должно быть понятно, что он делает. 
Например, если нам нужно получить из базы доктора по его id пишем findDoctorById(Long id). 
В методе должно быть отражено _кого_ мы получаем, и _как_ его нужно искать. По той же логике работает JPA.
- Ненужные ссылки.
