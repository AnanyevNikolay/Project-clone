...
public class EconomistCheckRestController {
    ...
    public Response<Void> checkPricesByYet(@RequestBody DatesToCheckDto datesToCheckDto) {
        ...
        //todo list1 вынести валидацию полей на null на уровень дто, поправить тесты и убедиться что все работает
        ...
        //todo list1 findAllYetsBetweenDayFromAndDayTo() находит уеты но не сортирует их, это делает checkIfThereAnyDateOverlap() второй итерацией, что делает код неэффективным. Необходимо сразу сортировать уеты в первом методе, а во втором вырезать сортировку
        ...
        //todo list1 аналогичная проблема с сортировкой
